...
class V2:
    def __init__(self, root, paper,v1_user,v1_correct,q1_user,q1_correct):
        self.root = root
        self.paper = paper
        ...
        self.v1_user_choice = v1_user
        self.v1_answer_choice = v1_correct
        self.q1_user_choice = q1_user
        self.q1_answer_choice = q1_correct
        ...
        self.pages = []
        self.v2_user_choice=['none']*15
        self.v2_answer_choice=['none']*15

        self.create_pages()

        # 添加倒计时
        # 初始化时间
        self.time_left = 23 * 60  # 18分钟的秒数
        # 创建标签显示剩余时间
    ...
    def create_one_choice_pages(self,page,question,choices):
        ...
        left_question_text.insert('1.0', f'Verbal question {page} of 15')
        ...
        def selection():
            ...
            self.v2_user_choice[page-1]=selected_value
    ...
    def create_three_choices_pages(self,i,question,choices):
        ...
        left_question_text = tk.Text(page_frame, bg="#F0E1E4", fg='black', font=('Arial', 18), wrap='word',
                                     borderwidth=0, highlightthickness=0)
        left_question_text.place(relx=0.000000, rely=0.072917, relwidth=0.292912, relheight=0.031250)
        left_question_text.insert('1.0', f'Verbal question {i} of 15')
        ...
        def update_three_blank_user_choice():
            ...
            try:
                ...
                self.v2_user_choice[i - 1] = selected_values
                self.change_page_buttons[i - 1].config(bg='green')
                print(self.v2_user_choice)
            except NameError:
                pass
    ...
    def create_six_choose_two_pages(self,page,question,choices):
        ...
        next_button.place(relx=0.937, rely=0.021, relwidth=0.03, relheight=0.03)
        # Question label
        left_question_text = tk.Text(page_frame, bg="#3b3736", fg='white', font=('Arial', 18), wrap='word',
                                     borderwidth=0, highlightthickness=0)
        left_question_text.place(relx=0.146456, rely=0.016667, relwidth=0.292912, relheight=0.041667)
        left_question_text.insert('1.0', f'Question {page} of 15')
        ...
        def show_selection():
            ...
            self.v2_user_choice[page-1] = select_text
            self.change_page_buttons[page - 1].config(bg='green')
    ...
    def go_to_q2(self):
        result = messagebox.askokcancel("确认", "你确定要进入Quantity2吗？")
        if result:
            for widget in self.root.winfo_children():
                widget.destroy()
            q2.Q2(self.root,self.paper,self.v1_user_choice,self.v1_answer_choice,self.q1_user_choice,self.q1_answer_choice,self.v2_user_choice,self.v2_answer_choice)


    def create_pages(self):

        for i in range(1,16):
            question_type = self.paper.get(f'section4 题目{i} 类型（单空，双空，三空，六选二，阅读单选，阅读多选，阅读选句子）')

            if question_type[:2]  == '单空':
                question = self.paper.get(f'section4 题目{i} 题干')
                choices = self.paper.get(f'section4 题目{i} 选项')
                answers = self.paper.get(f'section4 题目{i} 答案')
                self.v2_answer_choice[i-1]=answers
            ...
            if question_type[:4]  == '阅读单选' or question_type[:5]  == '阅读选句子':
                passage = self.paper.get(f'section4 题目{i} 阅读文章（没有为空）')
                question = self.paper.get(f'section4 题目{i} 题干')
                choices = self.paper.get(f'section4 题目{i} 选项')
                answers = self.paper.get(f'section4 题目{i} 答案')
                self.v2_answer_choice[i-1] = answers
    ...
    def on_time_up(self):
        for widget in self.root.winfo_children():
            widget.destroy()
        q2.Q2(self.root, self.paper, self.v1_user_choice, self.v1_answer_choice, self.q1_user_choice,
              self.q1_answer_choice, self.v2_user_choice, self.v2_answer_choice)
