...
class Q2:
    def __init__(self, root, paper,v1_user_choices,v1_right_choices,q1_user,q1_ans,v2_user,v2_ans):
        self.root = root
        self.paper = paper
        ...
        self.q1_user_choice = q1_user
        self.q1_answer_choice = q1_ans
        self.v2_user_choice = v2_user
        self.v2_answer_choice = v2_ans
        ...
        self.q2_user_choice = ['none'] * 15
        self.q2_answer_choice = ['none'] * 15
    ...
    def create_navigation_buttons(self):
        for index in range(len(self.pages)):
            ...
            change_page_button.place(relx=0.146456 + 0.029291 * index, rely=0.016667, relwidth=0.023433, relheight=0.041667)
    ...
    def create_one_choice_pages(self, i, diagram, choice_number):
        ...
        back_button.place(relx=0.878735, rely=0.020833, relwidth=0.029291, relheight=0.020833)
        next_button = tk.Button(page_frame, text="Next", command=self.go_next)
        next_button.place(relx=0.937317, rely=0.020833, relwidth=0.029291, relheight=0.020833)
        ...
        left_question_text.insert('1.0', f'Quantity question {i} of 15')
        ...
        def selection():
            ...
            self.q2_user_choice[i - 1] = selected_value
    ...
    def create_multiple_choices_pages(self, page, diagram, choice_number):
        ...
        left_question_text = tk.Text(page_frame, bg="#F0E1E4", fg='black', font=('Arial', 18), wrap='word',
                                     borderwidth=0, highlightthickness=0)
        left_question_text.place(relx=0.000000, rely=0.072917, relwidth=0.292912, relheight=0.031250)
        left_question_text.insert('1.0', f'Quantity question {page} of 15')
        ...
        def selection():
            ...
            self.q2_user_choice[page - 1] = select_text
        ...
        for i in range(choice_number):
            ...
            button = tk.Checkbutton(page_frame, text=abcdefgh[i], variable=var, command=selection,
                                    font=("Helvetica", 15), bg='white')
            button.place(relx=0.175747 + 0.0468658 * i, rely=0.781250)
        return page_frame
    ...
    def create_fill_blank_pages(self, page, diagram):
        ...
        def on_enter_pressed(event):
            ...
            self.q2_user_choice[page-1] = user_input
            self.change_page_buttons[page - 1].config(bg='green')
    ...
    def go_to_result(self):
        result = messagebox.askokcancel("确认", "你确定要结束考试吗？")
        if result:
            for widget in self.root.winfo_children():
                widget.destroy()
            generate_results.Generate_result(self.root,self.paper,self.v1_user_choice,self.v1_answer_choice,self.q1_user_choice,self.q1_answer_choice,self.v2_user_choice,self.v2_answer_choice,self.q2_user_choice,self.q2_answer_choice)


    def create_pages(self):

        for i in range(1, 16):
            question_type = self.paper.get(f'section5 题目{i} 类型（单选，多选，填空）')
            diagram  = self.paper.get(f'section5 题目{i} 题目（填路径）')
            choice_number = self.paper.get(f'section5 题目{i} 选项个数（填数字）')
            answers = self.paper.get(f'section5 题目{i} 答案')

            if question_type[:2] == '单选':
                self.q2_answer_choice[i - 1] = answers
